...
int ast_add_parameter(AstNode*f,const char*name){char**x=realloc(f->parameters,(f->parameter_count+1)*sizeof(*x));if(!x)return 0;x[f->parameter_count]=dup_text(name);if(!x[f->parameter_count])return 0;f->parameters=x;f->parameter_count++;return 1;}
