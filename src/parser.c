...
static AstNode *primary(Parser *p) {
    if (match(p, TOKEN_INTEGER)) {
        AstNode *n = ast_new(AST_INTEGER, p->previous.line);
        if (!n) return NULL;
        n->integer_value = strtol(p->previous.start, NULL, 10);
        return n;
    }
...
static AstNode *factor(Parser *p) {
    ...
    while (check(p, TOKEN_STAR) || check(p, TOKEN_SLASH)) {
        Token op = p->current; advance_token(p);
        AstNode *right = primary(p);
        if (!left || !right) { ast_free(left); ast_free(right); return NULL; }
        AstNode *n = ast_new(AST_BINARY, op.line);
        if (!n) { ast_free(left); ast_free(right); return NULL; }
        n->left = left; n->right = right; n->op = op.kind == TOKEN_STAR ? OP_MUL : OP_DIV; left = n;
    }
    return left;
}
...
static AstNode *expression(Parser *p) {
    ...
    while (check(p, TOKEN_PLUS) || check(p, TOKEN_MINUS)) {
        ...
        AstNode *n = ast_new(AST_BINARY, op.line);
        if (!n) { ast_free(left); ast_free(right); return NULL; }
        n->left = left; n->right = right; n->op = op.kind == TOKEN_PLUS ? OP_ADD : OP_SUB; left = n;
    }
    return left;
}
...
static AstNode *statement(Parser *p) {
    if (match(p, TOKEN_LET)) {
        ...
        Token name = p->current; advance_token(p);
        ...
        AstNode *n = ast_new(AST_LET, name.line);
        if (!n) { ast_free(value); return NULL; }
        n->name = token_text(name); n->left = value;
        return n;
    }
    if (match(p, TOKEN_RETURN)) {
        AstNode *value = expression(p);
        consume(p, TOKEN_SEMICOLON, "expected ';' after return value");
        AstNode *n = ast_new(AST_RETURN, p->previous.line); n->left = value; return n;
    }
...
static AstNode *function(Parser *p) {
    Token fn = p->previous;
    if (!check(p, TOKEN_IDENTIFIER)) { error(p, "expected function name"); return NULL; }
    Token name = p->current; advance_token(p);
    ...
    AstNode *f = ast_new(AST_FUNCTION, fn.line);
    if (!f) return NULL;
    f->name = token_text(name);
    while (!check(p, TOKEN_RBRACE) && !check(p, TOKEN_EOF)) {
        AstNode *s = statement(p);
        if (!s) { ast_free(f); return NULL; }
        if (!ast_add_child(f, s)) { ast_free(s); ast_free(f); return NULL; }
    }
...
AstNode *parser_parse(Parser *p) {
    ...
    while (!check(p, TOKEN_EOF)) {
        if (!match(p, TOKEN_FN)) { error(p, "expected 'fn' at top level"); ast_free(program); return NULL; }
        AstNode *f = function(p);
        if (!f || !ast_add_child(program, f)) { ast_free(f); ast_free(program); return NULL; }
    }
    return p->had_error ? (ast_free(program), NULL) : program;
}
