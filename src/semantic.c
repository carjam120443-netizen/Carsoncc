...
static int function_check(const AstNode*f){char**names=NULL;size_t count=0;int ok=1;for(size_t i=0;i<f->child_count;i++){const AstNode*n=f->children[i];if(n->kind==AST_LET){if(has_name(names,count,n->name)){fprintf(stderr,"CarsonCC: line %lu: variable '%s' already declared\n",(unsigned long)n->line,n->name);ok=0;}if(!expr(n->left,names,count))ok=0;char**p=__builtin_realloc(names,(count+1)*sizeof(*p));if(!p){ok=0;break;}names=p;names[count++]=n->name;}else if(n->kind==AST_RETURN){if(!expr(n->left,names,count))ok=0;}}
    if(count==0){__builtin_free(names);return ok;} __builtin_free(names); return ok;}
